 ...
 *   - setup_bin_paths() sets backend_exec to the current executable.
 ...
 * These live in the backend objects linked into libvpg.a.
 */
...
extern void PostgresSingleUserMain(int argc, char *argv[], const char *username)
	__attribute__((noreturn));
extern const char *vpg_get_exec_path(void);  /* from vpg.c */

/* Forward declaration so vpg.c can call us */
int vpg_initdb_run(int argc, char *argv[]);

/*
 * VPG: pid of the child we forked for the current popen, so pclose can waitpid.
 */
static pid_t vpg_child_pid = -1;

/*
 * VPG: Parse a shell command string built by initdb into an argv array.
 * Returns argc; caller must free argv.
 */
static int
vpg_parse_cmd(const char *cmd, char ***argv_out)
{
	/* Simple tokeniser: split on spaces, honour double-quoted tokens. */
	char	  **av = NULL;
	int			ac = 0, cap = 0;
	const char *p = cmd;
	...
	while (*p)
	{
		...
		/* Skip shell redirects (< ... or > ...) — not needed in-process */
		if (*p == '<' || *p == '>') {
			while (*p && *p != ' ') p++;
			continue;
		}
...
/*
 * VPG: replacement for popen(cmd, "w").
 * Forks a child; the child connects its stdin to the read end of a pipe
 * and calls BootstrapModeMain or PostgresSingleUserMain directly.
 * The parent returns a FILE* wrapping the write end of the pipe.
 */
static FILE *
vpg_popen_w(const char *cmd)
{
	int   pipefd[2];
	pid_t pid;
	char **av;
	...
	if (pipe(pipefd) < 0)
	{
		fprintf(stderr, "vpg_initdb: pipe() failed: %s\n", strerror(errno));
		return NULL;
	}

	ac = vpg_parse_cmd(cmd, &av);

	pid = fork();
	if (pid < 0)
	{
		fprintf(stderr, "vpg_initdb: fork() failed: %s\n", strerror(errno));
		close(pipefd[0]);
		close(pipefd[1]);
		return NULL;
	}
	...
	if (pid == 0)
	{
		/* Child: wire stdin to pipe read end */
		close(pipefd[1]);
		 ...
		 * av[0] is backend_exec (path to "postgres"), av[1] is the flag.
		 */
		if (ac > 1 && strcmp(av[1], "--boot") == 0)
			BootstrapModeMain(ac, av, false);   /* noreturn */

		if (ac > 1 && strcmp(av[1], "--single") == 0)
		{
			const char *su = "postgres";
			for (int i = 1; i < ac - 1; i++) {
				if (strcmp(av[i], "-u") == 0 && i + 1 < ac - 1)
					su = av[i + 1];
			}
			PostgresSingleUserMain(ac, av, su);  /* noreturn */
		}

		fprintf(stderr, "vpg_initdb: unknown dispatch in popen cmd: %s\n", cmd);
		_exit(1);
	}
...
/*
 * VPG: replacement for pclose().
 * Closes the write end of the pipe (signalling EOF to the child) and
 * waits for the child to finish.  Returns 0 on success, -1 on error.
 */
static int
vpg_pclose_w(FILE *f)
{
	int   status;
	pid_t pid = vpg_child_pid;
	...
	{
		fprintf(stderr, "vpg_initdb: waitpid failed: %s\n", strerror(errno));
		return -1;
	}
	if (!WIFEXITED(status) || WEXITSTATUS(status) != 0)
	{
		fprintf(stderr, "vpg_initdb: child exited with status %d\n",
...
popen_check(const char *command, const char *mode)
{
	...
	fflush(NULL);
	/* VPG: always mode "w"; call in-process fork instead of popen() */
	(void) mode;
	cmdfd = vpg_popen_w(command);
	if (cmdfd == NULL)
		pg_log_error("could not execute command \"%s\": %m", command);
	return cmdfd;
}

/* VPG: local pclose_check shadows libpgcommon version; uses vpg_pclose_w */
static int vpg_local_pclose_check(FILE *stream);

#define pclose_check vpg_local_pclose_check

static int
vpg_local_pclose_check(FILE *stream)
{
	int rc = vpg_pclose_w(stream);
	if (rc != 0)
		pg_log_error("child process exited with error");
	return rc;
}
...
/*
 * VPG: Determine platform-specific config settings.
 * In embedded single-process mode we skip the postgres --check probing
 * and use fixed defaults that are appropriate for one connection at a time.
 */
static void
test_config_settings(void)
{
	...
	n_connections = 10;
	n_av_slots    = 3;
	n_buffers     = 1024;   /* 8 MB — small footprint for embedded use */

	printf(_("selecting dynamic shared memory implementation ... %s\n"),
		   dynamic_shared_memory_type);
	printf(_("selecting default \"max_connections\" ... %d\n"), n_connections);
	printf(_("selecting default \"shared_buffers\" ... %dkB\n"),
...
/* VPG: test_specific_config_settings removed — test_config_settings is hardcoded */
...
{
	/*
	 * VPG: No external postgres binary needed — we call BootstrapModeMain /
	 * PostgresSingleUserMain directly in-process.  Set backend_exec to the
	 * current executable so share_path resolution still works.
	 */
	strlcpy(backend_exec, vpg_get_exec_path(), MAXPGPATH);
